...
def find_event_files():
    """
    Return event files together with their storage partition.
...
def load_valid_event_file(
    ...
    if "match_id" not in frame.columns:
        log(
            f"[ERROR] Event file has no match_id: "
            f"{filepath}"
        )
        return pd.DataFrame()
...
def inspect_item_events(files):
    """
    Find Mejai-related item events while excluding invalid
    matches before catalogue construction.
    """
    ...
    for file_number, (
        ...
        candidates = find_mejai_candidates(
            frame
        )
        ...
        candidates["source_file"] = str(
            filepath
        )

        mejai_event_frames.append(
            candidates
        )
        ...
        if (
            ...
            current_count = sum(
                len(candidate_frame)
                for candidate_frame
                in mejai_event_frames
            )

            log(
                f"  Mejai scan: "
                f"{file_number:,}/{len(files):,} files | "
                f"{current_count:,} candidate events"
            )
...
def save_catalogue(catalogue):
    MEJAI_EVENT_CATALOGUE.parent.mkdir(
        parents=True,
        exist_ok=True,
    )

    temporary_path = Path(
        str(MEJAI_EVENT_CATALOGUE)
        + ".tmp"
    )
...
def print_candidate_examples(
    ...
    if len(mejai_events) > 50:
        ...
        log(
            f"Showing 50 of "
            f"{len(mejai_events):,} events."
        )


def print_mejai_event_summary(
    mejai_events,
):
    log("")
    log(
    ...
    log("")
    log(
        "Mejai matches by storage partition:"
    )
...
def main():
    ...
    if not files:
        log(
            "[ERROR] No event Parquet files found"
        )
        return
    ...
    event_type_counts = inspect_event_types(
        files
    )
    ...
    mejai_events = inspect_item_events(
        files
    )

    log(
        f"Mejai candidate events: "
        f"{len(mejai_events):,}"
    )
    ...
    if mejai_events.empty:
        log("")
        log(
            "[WARNING] No Mejai candidates found."
        )
        return

    mejai_events = (
        mejai_events.sort_values(
            [
                "match_id",
                "participant_id",
                "timestamp",
    ...
    log("")
    log(
        f"Catalogue written to: "
        f"{MEJAI_EVENT_CATALOGUE}"
    )

    log(
        f"Total candidate events: "
        f"{len(catalogue):,}"
    )

    log("")
    log(
        "[PASSED] MEJAI EVENT CATALOGUE REBUILT "
        "FROM ELIGIBLE MATCHES"
    )
