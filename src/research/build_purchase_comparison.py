...
import numpy as np
import pandas as pd
from src.research.config import LIFECYCLE_FILE, V2_CONTROL_POOL
from src.research.utils import determine_lane, load_research_raw_data, load_control_raw_data
MAX_TIME_GAP_MS = 60000
MAX_CONTROLS_PER_CASE = 30
MAX_LEVEL_GAP = 1
# Primary candidate calipers.
MAX_PLAYER_GOLD_GAP_RATIO = 0.25
MAX_PLAYER_GOLD_DIFF_VS_ROLE_GAP = 2500
MAX_REST_OF_TEAM_GOLD_DIFF_GAP = 4000
# XP-based state is used for ranking and diagnostics rather than hard exclusion.

PLAYER_XP_DIFF_SCORE_SCALE = 2500
REST_OF_TEAM_XP_DIFF_SCORE_SCALE = 4000
TIME_GAP_SCORE_SCALE_MS = 60000
TIME_BUCKET_MS = 30000
...
def prepare_lifecycles(df):
    ...
    df = df.dropna(subset=['match_id', 'participant_id', 'purchase_timestamp', 'status'])
...
def prepare_snapshots(snapshots, participants):
    ...
    snapshots = snapshots.dropna(subset=['match_id', 'participant_id', 'timestamp'])
    ...
    if not participants.empty:
        ...
        for column in ['team_id', 'team_position', 'champion_id', 'champion_name', 'win']:
            if column in participants.columns:
                metadata_columns.append(column)
        metadata = participants[metadata_columns].drop_duplicates(subset=['match_id', 'participant_id'])
        already_attached = [
            column
...
def add_team_state_features(snapshots):
    ...
    snapshots['participant_id'] = pd.to_numeric(snapshots['participant_id'], errors='coerce')
    ...
    snapshots = snapshots.sort_values(
        ["match_id", "timestamp", "participant_id", "role_opponent_participant_id"],
        kind="stable",
    ).drop_duplicates(
...
def build_snapshot_lookup(snapshots):
    if snapshots.empty:
        return {}
    snapshots = snapshots.sort_values(['match_id', 'participant_id', 'timestamp'])
    return {
...
def get_player_snapshot(snapshot_lookup, match_id, participant_id, target_timestamp):
    ...
    position = np.searchsorted(timestamps, int(target_timestamp), side='right') - 1
    if position < 0:
        return None
...
def build_control_index(control_snapshots):
    ...
    return indexed.groupby(['team_position', 'level', 'time_bucket'], sort=False, observed=True).indices

def get_control_row_indices(control_index, team_position, case_level, target_timestamp):
    minimum_bucket = max(0, (int(target_timestamp) - MAX_TIME_GAP_MS) // TIME_BUCKET_MS)
    maximum_bucket = int(target_timestamp) // TIME_BUCKET_MS
    index_parts = []

    for level in range(max(1, int(case_level) - MAX_LEVEL_GAP), int(case_level) + MAX_LEVEL_GAP + 1):
...
def score_control_candidates(case_snapshot, candidates, target_timestamp):
    ...
    missing = [column for column in required_control_columns if column not in candidates.columns]
    if missing:
        return pd.DataFrame()
    ...
    time_gap = int(target_timestamp) - candidates["timestamp"].to_numpy(dtype=float)
    ...
    return candidates.nsmallest(MAX_CONTROLS_PER_CASE, 'control_match_state_score')
...
def build_output_row(mejai_case, case_snapshot, control_snapshot):
    ...
    row = {
        "case_id": f"{case_match_id}_{case_participant_id}_{purchase_timestamp}",
        ...
        "control_snapshot_age_ms": int(control_snapshot["control_snapshot_age_ms"]),
        "control_time_seconds": float(control_snapshot["timestamp"]) / 1000,
        "control_match_state_score": float(control_snapshot["control_match_state_score"]),
        "control_group": 1,
...
def build_control_dataset(lifecycles, case_data, control_data):
    ...
    for lane, case_tables in case_data.items():
        if lane not in control_data:
            continue
        lane_cases = lifecycles[lifecycles['match_id'].map(determine_lane) == lane]
        if lane_cases.empty:
            continue
        log('')
        ...
        case_snapshots = prepare_snapshots(case_tables['snapshots'], case_participants)
        ...
        # Prepare control snapshots and vectorized team state.
        ...
        log(f"Preparing vectorized control state for "f"{len(control_snapshots):,} snapshots...")
        ...
        missing = [column for column in required_control_columns if column not in control_snapshots.columns]
        if missing:
        ...
        log(
            f"Building control index for "
            f"{len(control_snapshots):,} usable snapshots..."
        )

        control_index = build_control_index(control_snapshots)
        log(f'Control index buckets: {len(control_index):,}')
        # Match each Mejai purchase to eligible control snapshots.
        for _, mejai_case in lane_cases.iterrows():
            ...
            if case_snapshot is None:
                continue
            snapshot_age = int(mejai_case['purchase_timestamp']) - int(case_snapshot['timestamp'])
            ...
            for _, control_snapshot in matches.iterrows():
                output_rows.append(
                    build_output_row(mejai_case, case_snapshot, control_snapshot)
                )
        ...
        # Release lane-specific structures before moving to the next region.
        del control_index
...
def save_dataset(df):
    ...
    log(f'[SAVED] Control candidates written to: {V2_CONTROL_POOL}')
...
def main():
    ...
    controls = build_control_dataset(lifecycles, case_data, control_data)
    if controls.empty:
        log('[ERROR] No control candidates could be constructed')
        return
    controls = controls.drop_duplicates(
        subset=["case_id", "control_match_id", "control_participant_id"]
    ).reset_index(drop=True)
