...
def load_valid_match_manifest():
    ...
    manifest = pd.read_parquet(VALID_MATCH_MANIFEST, engine="pyarrow").copy()
    required_columns = {"match_id", "storage_partition", "analysis_eligible"}
    missing_columns = sorted(required_columns - set(manifest.columns))
    if missing_columns:
        raise ValueError(f"Valid-match manifest is missing columns: {missing_columns}")
    
    manifest["match_id"] = manifest["match_id"].astype(str)
    manifest["storage_partition"] = manifest["storage_partition"].astype(str).str.strip().str.lower()
    manifest["analysis_eligible"] = manifest["analysis_eligible"].astype("boolean").fillna(False).astype(bool)
    duplicate_match_ids = manifest["match_id"].duplicated(keep=False)

    if duplicate_match_ids.any():
        duplicate_examples = manifest.loc[duplicate_match_ids, ["match_id", "storage_partition"]].head(20)
        raise ValueError(
...
def get_valid_match_ids(lane=None):
    ...
    if lane is not None:
        ...
        if lane not in LANES:
            raise ValueError(f"Unknown storage partition: {lane}")
        eligible = eligible[eligible["storage_partition"] == lane]
    return frozenset(eligible["match_id"])
...
def find_files(table, lane):
    ...
    if not directory.exists():
        return []
    return sorted(directory.glob(f"{lane}*part*.parquet"))
...
def get_parquet_columns(filepath):
    ...
    """
    try:
        return set(pq.ParquetFile(filepath).schema_arrow.names)
    except Exception as error:
        raise RuntimeError(f"Could not inspect Parquet schema for {filepath}: {error}") from error
...
def load_table_for_lane(table, lane, columns=None, match_ids=None):
    ...
    if match_ids is not None:
        normalized_match_ids = {str(match_id) for match_id in match_ids}
    ...
    for filepath in files:
        try:
            ...
            if normalized_match_ids is not None:
                filters = [("match_id", "in", list(normalized_match_ids))]

            if columns is not None:
                available_columns = get_parquet_columns(filepath)
...
def filter_to_match_ids(df, match_ids):
    ...
    if "match_id" not in df.columns:
        raise ValueError("Cannot filter table because match_id is missing")
...
def load_research_raw_data(lifecycles, include_events=True):
    if lifecycles.empty:
        return {}
    lifecycle_match_ids = {str(match_id) for match_id in lifecycles["match_id"]}
    all_valid_match_ids = get_valid_match_ids()
    invalid_lifecycle_match_ids = lifecycle_match_ids - all_valid_match_ids

    if invalid_lifecycle_match_ids:
        log("")
        log(f"Lifecycle matches excluded by valid-match manifest: {len(invalid_lifecycle_match_ids):,}")
        for match_id in sorted(invalid_lifecycle_match_ids)[:20]:
            log(f"  excluded: {match_id}")

    data = {}
    for lane in LANES:
        valid_lane_match_ids = get_valid_match_ids(lane)
        lane_match_ids = lifecycle_match_ids & valid_lane_match_ids
        if not lane_match_ids:
            continue

        log("")
        log(f"========== LOADING {lane.upper()} ==========")
        log(f"Eligible lifecycle matches requested: {len(lane_match_ids):,}")
...
def load_control_raw_data(lifecycles):
    ...
    if lifecycles.empty:
        return {}
    mejai_match_ids = {str(match_id) for match_id in lifecycles["match_id"]}
    data = {}
    for lane in LANES:
        log("")
        log(f"========== LOADING {lane.upper()} CONTROLS ==========")
        matches = load_table_for_lane(MATCH_TABLE, lane)
        matches = prepare_matches(matches)
        if matches.empty:
            log(f"[WARNING] No match table found for {lane}")
            continue

        valid_lane_match_ids = get_valid_match_ids(lane)
        raw_lane_match_ids = set(matches["match_id"].astype(str))
        eligible_lane_match_ids = raw_lane_match_ids & valid_lane_match_ids
        excluded_match_ids = raw_lane_match_ids - eligible_lane_match_ids

        lane_mejai_match_ids = eligible_lane_match_ids & mejai_match_ids
        control_match_ids = eligible_lane_match_ids - mejai_match_ids

        log(f"Raw matches available: {len(raw_lane_match_ids):,}")
        log(f"Matches excluded by manifest: {len(excluded_match_ids):,}")
        log(f"Eligible matches available: {len(eligible_lane_match_ids):,}")
        log(f"Eligible matches containing Mejai: {len(lane_mejai_match_ids):,}")
        log(f"Eligible non-Mejai control matches: {len(control_match_ids):,}")

        if excluded_match_ids:
            for match_id in sorted(excluded_match_ids)[:10]:
                log(f"  excluded: {match_id}")
        if not control_match_ids:
            continue

        # Load participant rows only for eligible non-Mejai matches.
        participants = load_table_for_lane(PARTICIPANT_TABLE, lane, match_ids=control_match_ids)
        participants = prepare_participants(participants)
        participants = filter_to_match_ids(participants, control_match_ids)

        # Load snapshot rows only for eligible non-Mejai matches.
        snapshots = load_table_for_lane(SNAPSHOT_TABLE, lane, match_ids=control_match_ids)
        snapshots = prepare_snapshots(snapshots)
        snapshots = filter_to_match_ids(snapshots, control_match_ids)
        eligible_matches = filter_to_match_ids(matches, control_match_ids)
        
        log(f"Control matches loaded: {len(eligible_matches):,}")
        log(f"Control participants loaded: {len(participants):,}")
        log(f"Control snapshots loaded: {len(snapshots):,}")
