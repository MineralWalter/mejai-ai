...
def validate_duplicates(df):
    ...
    log(
        f"Duplicate case_id rows: "
        f"{duplicate_cases:,}"
    )

    log(
        f"Completely duplicate rows: "
        f"{duplicate_rows:,}"
    )
...
def validate_snapshot_age(df):
    ...
    log(
        f"Negative snapshot ages: "
        f"{invalid_negative:,}"
    )

    log(
        f"Snapshot ages > 60 seconds: "
        f"{over_60:,}"
    )

    if invalid_negative == 0:
        log(
            "[PASSED] No future snapshots"
        )

    if over_60 == 0:
        log(
            "[PASSED] All snapshots "
            "within expected 60-second cadence"
        )
...
def validate_temporal_order(df):
    ...
    if invalid == 0:
        log(
            "[PASSED] All snapshots occur "
            "at or before purchase"
        )
...
def validate_team_consistency(df):
    ...
    log(
        "Research cases per match/team:"
    )

    log(
        match_team_cases.describe().to_string()
    )

    # --------------------------------------------------------
    # TEAM / ENEMY GOLD CONSISTENCY
    # --------------------------------------------------------

    calculated_total_gold_diff = (
    ...
    log(
        f"Team total-gold difference mismatches: "
        f"{total_gold_mismatch:,}"
    )

    # --------------------------------------------------------
    # TEAM / ENEMY CURRENT GOLD CONSISTENCY
    # --------------------------------------------------------

    calculated_current_gold_diff = (
        df["team_current_gold_sum"]
        - df["enemy_current_gold_sum"]
    )

    current_gold_mismatch = (
        calculated_current_gold_diff
        != df["team_current_gold_diff"]
    ).sum()

    log(
        f"Team current-gold difference mismatches: "
        f"{current_gold_mismatch:,}"
    )

    # --------------------------------------------------------
    # TEAM CASE REPRESENTATION
    # --------------------------------------------------------
    ...
    log(
        f"Match/team groups with inconsistent "
