...
CATEGORICAL_COVARIATES = ['region', 'team_position', 'champion_name']
GOOD_SMD_THRESHOLD = 0.1
WARNING_SMD_THRESHOLD = 0.2
...
def standardised_mean_difference(case_mean, control_mean, case_variance, control_variance):
...
def load_matched_dataset(sample_name, filepath):
    if not filepath.exists():
        raise FileNotFoundError(f'{sample_name} matched dataset not found: {filepath}')
    df = pd.read_parquet(filepath, engine='pyarrow').copy()
    required_columns = ['matched_set_id', 'treatment', 'matching_weight', 'outcome_win', 'observation_timestamp']
    ...
    df['observation_timestamp'] = pd.to_numeric(df['observation_timestamp'], errors='coerce')
    if df[['treatment', 'matching_weight', 'observation_timestamp']].isna().any(axis=None):
        raise ValueError(f'{sample_name} contains invalid treatment, weight, or observation timestamp values')

    df['treatment'] = df['treatment'].astype(int)
    df['observation_time_minutes'] = df['observation_timestamp'] / 60000
    validate_matched_structure(df, sample_name)
    return df

def validate_matched_structure(df, sample_name):
    if not df['treatment'].isin([0, 1]).all():
        raise ValueError(f'{sample_name}: treatment must contain only 0 and 1')

    if (df['matching_weight'] <= 0).any():
        raise ValueError(f'{sample_name}: matching weights must be positive')
    set_summary = df.groupby('matched_set_id').agg(case_rows=('treatment', 'sum'), total_rows=('treatment', 'size'))
    ...
    if not set_summary['total_rows'].between(2, 4).all():
        raise ValueError(f'{sample_name}: every set must contain one to three controls')
    
    case_weights = df[df['treatment'] == 1].groupby('matched_set_id')['matching_weight'].sum()
    control_weights = df[df['treatment'] == 0].groupby('matched_set_id')['matching_weight'].sum()

    if not np.isclose(case_weights, 1.0).all():
        raise ValueError(f'{sample_name}: case weights do not sum to one per set')
    
    if not np.isclose(control_weights, 1.0).all():
        raise ValueError(f'{sample_name}: control weights do not sum to one per set')

def paired_set_differences(df, column):
    working = df[['matched_set_id', 'treatment', 'matching_weight', column]].copy()
    working[column] = pd.to_numeric(working[column], errors='coerce')

    case_values = working[working['treatment'] == 1].set_index('matched_set_id')[column]
    controls = working[working['treatment'] == 0].dropna(subset=[column])

    if controls.empty:
        return pd.Series(dtype=float)
    
    controls['weighted_value'] = controls[column] * controls['matching_weight']
    control_means = controls.groupby("matched_set_id").agg(
        weighted_sum=("weighted_value", "sum"),
        weight_sum=("matching_weight", "sum"),
    )

    control_means['control_mean'] = control_means['weighted_sum'] / control_means['weight_sum']
    paired = pd.concat([case_values.rename('case_value'), control_means['control_mean']], axis=1, join='inner').dropna()
...
def calculate_numeric_balance(df, sample_name):
    ...
    available_columns = [column for column in NUMERIC_COVARIATES if column in df.columns]
    for column in available_columns:
        case_values = pd.to_numeric(case_rows[column], errors='coerce')
        control_values = pd.to_numeric(control_rows[column], errors='coerce')

        case_valid = case_values.notna()
        control_valid = control_values.notna()

        case_mean = weighted_mean(case_values[case_valid], case_rows.loc[case_valid, 'matching_weight'])
        control_mean = weighted_mean(control_values[control_valid], control_rows.loc[control_valid, 'matching_weight'])

        case_variance = weighted_variance(case_values[case_valid], case_rows.loc[case_valid, 'matching_weight'])
        control_variance = weighted_variance(
            control_values[control_valid],
            control_rows.loc[control_valid, "matching_weight"],
        )

        smd = standardised_mean_difference(case_mean, control_mean, case_variance, control_variance)
    ...
    if not rows:
        raise ValueError(f'{sample_name}: no expected numeric covariates were found')
    return pd.DataFrame(rows).sort_values(['absolute_smd', 'covariate'], ascending=[False, True]).reset_index(drop=True)

def weighted_category_proportions(df, column, treatment):
    sample = df[df['treatment'] == treatment][[column, 'matching_weight']].copy()
    ...
    return sample.groupby(column)['matching_weight'].sum().div(total_weight)
...
def calculate_categorical_balance(df, sample_name):
    ...
    for column in CATEGORICAL_COVARIATES:
        ...
        case_proportions = weighted_category_proportions(df, column, treatment=1)
        control_proportions = weighted_category_proportions(df, column, treatment=0)
        categories = sorted(set(case_proportions.index) | set(control_proportions.index))
    ...
    if not details.empty:
        details = details.sort_values(
            ["sample", "covariate", "absolute_proportion_difference"],
...
def calculate_set_consistency(df, sample_name):
    rows = []
    fields = ['region', 'team_position', 'dark_seal_purchased_before_observation']
    for field in fields:
        if field not in df.columns:
            continue

        unique_values = df.groupby('matched_set_id')[field].nunique(dropna=False)
...
def build_sample_summary(df, sample_name, numeric_balance, categorical_summary, consistency):
    case_rows = df[df['treatment'] == 1]
    control_rows = df[df['treatment'] == 0]

    smd_values = numeric_balance['absolute_smd'].replace([np.inf, -np.inf], np.nan)
    poor_covariates = numeric_balance[numeric_balance['absolute_smd'] >= WARNING_SMD_THRESHOLD]
    ...
    if not categorical_summary.empty:
        champion_rows = categorical_summary[categorical_summary['covariate'] == 'champion_name']
        if not champion_rows.empty:
            champion_tvd = float(champion_rows.iloc[0]['total_variation_distance'])
    exact_field_mismatches = int(consistency['mismatched_sets'].sum()) if not consistency.empty else 0
    return {
        "sample": sample_name,
...
def print_sample_results(summary, numeric_balance, consistency):
    ...
    log(f"Control effective sample size: {summary['control_effective_sample_size']:,.1f}")
    log(f"Maximum absolute SMD: {summary['maximum_absolute_smd']:.4f}")
    log(f"Median absolute SMD: {summary['median_absolute_smd']:.4f}")

    log(f"Covariates below 0.10: {summary['covariates_good_below_0_10']:,}")
    log(f"Covariates from 0.10 to below 0.20: {summary['covariates_review_0_10_to_0_20']:,}")
    log(f"Covariates at or above 0.20: {summary['covariates_poor_0_20_or_more']:,}")
    ...
    if not consistency.empty:
        ...
        log(consistency[['field', 'mismatched_sets', 'mismatch_rate']].to_string(index=False))
...
def write_text_report(summaries, numeric_results, consistency_results):
    ...
    for summary in summaries:
        ...
        sample_numeric = numeric_results[numeric_results['sample'] == sample]
        sample_consistency = consistency_results[consistency_results['sample'] == sample]
        lines.extend(
            [
                sample.upper(),
                "-" * 72,
                ...
                f"Maximum absolute SMD: {summary['maximum_absolute_smd']:.4f}",
                f"Median absolute SMD: {summary['median_absolute_smd']:.4f}",
                f"Covariates below 0.10: {summary['covariates_good_below_0_10']:,}",
                (
        ...
        if not sample_consistency.empty:
            lines.extend(
                [
                    ...
                    sample_consistency[
                        ["field", "mismatched_sets", "mismatch_rate"]
                    ].to_string(index=False),
                    "",
                ]
            )
...
def main():
    ...
    for sample_name, filepath in MATCHED_FILES.items():
        ...
        numeric_balance = calculate_numeric_balance(df, sample_name)
        categorical_summary, categorical_details = calculate_categorical_balance(df, sample_name)
        consistency = calculate_set_consistency(df, sample_name)
        summary = build_sample_summary(df, sample_name, numeric_balance, categorical_summary, consistency)

        print_sample_results(summary, numeric_balance, consistency)

        numeric_balance.to_csv(OUTPUT_DIR / f'{sample_name}_numeric_balance.csv', index=False)
        categorical_summary.to_csv(OUTPUT_DIR / f'{sample_name}_categorical_balance_summary.csv', index=False)
        categorical_details.to_csv(OUTPUT_DIR / f'{sample_name}_categorical_balance_details.csv', index=False)
        consistency.to_csv(OUTPUT_DIR / f'{sample_name}_set_consistency.csv', index=False)
    ...
    categorical_summary_results = pd.concat(all_categorical_summary, ignore_index=True)
    categorical_detail_results = pd.concat(all_categorical_details, ignore_index=True)
    consistency_results = pd.concat(all_consistency, ignore_index=True)

    summary_results = pd.DataFrame(summaries)

    numeric_results.to_csv(OUTPUT_DIR / 'all_numeric_balance.csv', index=False)
    categorical_summary_results.to_csv(OUTPUT_DIR / 'all_categorical_balance_summary.csv', index=False)
    categorical_detail_results.to_csv(OUTPUT_DIR / 'all_categorical_balance_details.csv', index=False)
    consistency_results.to_csv(OUTPUT_DIR / 'all_set_consistency.csv', index=False)

    summary_results.to_csv(OUTPUT_DIR / 'balance_summary.csv', index=False)
    report_file = write_text_report(summaries, numeric_results, consistency_results)
