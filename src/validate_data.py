...
def validate_batches(results):
    ...
    for lane in LANES:
        ...
        table_batches = {
            table: get_batch_ids(table, lane)
            for table in TABLES
        }

        all_batches = sorted(
            set().union(*table_batches.values())
        )

        if not all_batches:
            results["errors"].append(
                f"No Parquet batches found for {lane}"
            )
            continue

        expected_batches = list(
            range(min(all_batches), max(all_batches) + 1)
        )

        # Missing batch numbers

        missing_batch_numbers = sorted(
            set(expected_batches) - set(all_batches)
        )

        if missing_batch_numbers:
            message = (
                f"{lane}: missing batch numbers "
                f"{missing_batch_numbers}"
            )

            log(f"[ERROR] {message}")
            results["errors"].append(message)

        # Check that all four tables have the same batches
        ...
        for batch_id in all_batches:
            ...
            results["batch_counts"].append({
                "lane": lane,
                "batch_id": batch_id,
                "counts": counts,
            })

            # A normal full batch should contain 100 matches, use the matches table for this check.
            ...
            if match_count == 0:
                ...
                message = (
                    ...
                    f"(more than batch size {BATCH_SIZE})"
                )
                ...
                # This is not necessarily an error because the
                # final batch of a lane can naturally be smaller.
                log(
                    f"[INFO] {lane} batch {batch_id:05d}: "
                    f"{match_count} matches"
                )
...
def validate_schema(results):
    ...
    # No hardcoding, just record the columns and dtypes for each table/lane/batch combination.
        ...
        for table in TABLES:
            ...
            for batch_id in batch_ids:
                ...
                if not isinstance(df, pd.DataFrame):
                    message = (
                        f"{lane} {table} batch "
                        f"{batch_id:05d}: invalid DataFrame"
                    )
    ...
    for lane in LANES:
        ...
        for batch_id in get_batch_ids("matches", lane):
            ...
            if match_column is None:
                message = (
                    f"{lane} batch {batch_id:05d}: "
                    f"could not find match ID column"
                )
...
def validate_participants(results):
    ...
    for lane in LANES:
        ...
        for batch_id in get_batch_ids("matches", lane):
            ...
            participant_match_column = find_match_id_column(
                participants
            )

            if (
                match_column is None
                or participant_match_column is None
            ):
                log(
                    f"[WARNING] {lane} batch "
                    f"{batch_id:05d}: "
                    f"could not determine match ID columns"
                )
                continue

            participant_counts = (
                participants
                .groupby(participant_match_column)
                .size()
            )
            ...
            if not bad_counts.empty:
                ...
                message = (
                    f"{lane} batch {batch_id:05d}: "
                    f"{len(bad_counts)} matches do not "
                    f"have exactly 10 participants"
                )
            ...
            # Check participant match IDs actually exist.

            match_ids = set(
                matches[match_column].dropna()
            )

            participant_match_ids = set(
                participants[
                    participant_match_column
                ].dropna()
            )

            orphan_ids = (
                participant_match_ids - match_ids
            )
            ...
            if orphan_ids:
                ...
                message = (
                    f"{lane} batch {batch_id:05d}: "
                    f"{len(orphan_ids)} participant "
                    f"match IDs have no matching match row"
                )
    ...
    for lane in LANES:
        ...
        for table in TABLES:
            ...
            for batch_id in get_batch_ids(table, lane):
                ...
                # -------------------------------------------------
                # Detect nested / unhashable columns
                # -------------------------------------------------
...
def build_summary(results):
    ...
    # Count unique lane/batch combinations.
    batch_pairs = {
        (
            item["lane"],
            item["batch_id"]
        )
        for item in results["batch_counts"]
    }
...
def main():
    ...
    if not PARQUET_DIR.exists():
        ...
        log(
            f"[ERROR] Parquet directory does not exist: "
            f"{PARQUET_DIR}"
        )
    ...
    if results["warnings"]:
        ...
        log(
            f"[WARNING] "
            f"{len(results['warnings'])} warnings found"
        )
